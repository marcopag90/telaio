...
| `DalAuditOutcomeClassifier`   | SPI: maps exceptions to outcomes. With Spring Security on the classpath, `SecurityDalAuditOutcomeClassifier` maps `AccessDeniedException` and the RBAC field rejections (`DalFilterFieldNotReadableException` / `DalSortFieldNotReadableException` — thrown only for fields that exist and are hidden; a nonexistent field is a plain VALIDATION) → DENIED and everything else via the shared `DalFailureKind` taxonomy (VALIDATION / NOT_FOUND / CONFLICT / ERROR); without it, `DefaultDalAuditOutcomeClassifier` applies the same taxonomy minus DENIED |
