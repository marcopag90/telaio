...
| Type                          | Purpose                                                                                                                                                                                                                                                                                                     |
|-------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `DalRbacAdapter<T>`           | SPI: `Map<String,Object> filterInput(DalOperationType, Map<String,Object>, Authentication)`, `Object filterOutput(DalOperationType, T, Authentication)` and `boolean canFilterOn(String fieldPath, Authentication)` — may the principal reference this field in a `q=` filter or as a `sort=` key? (all `default` pass-through). A rejected field that does not exist on the entity falls through to the read's own validation (audit VALIDATION, not DENIED) |
